...
def build_model(embedding_dim, hidden_size, drop, sequence_length, vocabulary_size):
    ...
    tconv1 = TCN(nb_filters=hidden_size, dilations=[
                 1, 2, 4, 8], return_sequences=True, dropout_rate=drop, name='TCN1')(embedding)
    lstm1 = LSTM(units=hidden_size, dropout=drop, recurrent_dropout=drop,
                 return_sequences=True)(embedding)

    # outputs = Dense(units=vocabulary_size, activation='softmax')(concatenate([tconv1, lstm1, embedding]))
    outputs = Conv1D(vocabulary_size, 1, activation='softmax')(
        concatenate([tconv1, lstm1, embedding]))
...
def main(opt):
    ...
    if opt.mode == "train":
        ...
        tb = TensorBoard(log_dir='./logs/'+curtime, histogram_freq=1,
